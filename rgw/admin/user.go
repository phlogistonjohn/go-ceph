...
type User struct {
	UserID      string        `json:"user_id" url:"uid"`
...
func (api *API) GetUser(ctx context.Context, user User) (*User, error) {
	if user.UserID == "" {
		return nil, errors.New("missing user UID")
	}

	body, err := api.callAPI(ctx, get, "/user", valueToURLParams(user))
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("%s. %s. %w", unmarshalWrapError, string(body), err)
	}
...
func (api *API) GetUsers(ctx context.Context) (*[]string, error) {
	body, err := api.callAPI(ctx, get, "/metadata/user", nil)
	if err != nil {
		return nil, err
	}
	var users *[]string
...
func (api *API) CreateUser(ctx context.Context, user User) (*User, error) {
	...
	if user.DisplayName == "" {
		return nil, errors.New("missing user display name")
	}

	// Send request
	body, err := api.callAPI(ctx, put, "/user", valueToURLParams(user))
	if err != nil {
		return nil, err
	}
...
func (api *API) RemoveUser(ctx context.Context, user User) error {
	if user.UserID == "" {
		return errors.New("missing user UID")
	}

	_, err := api.callAPI(ctx, delete, "/user", valueToURLParams(user))
	if err != nil {
		return err
	}
...
func (api *API) ModifyUser(ctx context.Context, user User) (*User, error) {
	...
	body, err := api.callAPI(ctx, post, "/user", valueToURLParams(user))
	if err != nil {
		return nil, err
	}
